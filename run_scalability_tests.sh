#!/bin/bash
# Script para ejecutar pruebas de escalabilidad de KNN paralelo en Khipu
# Uso: bash run_scalability_tests.sh

echo "==================================================================="
echo "Pruebas de Escalabilidad - KNN Paralelo con MPI"
echo "==================================================================="
echo ""

# Crear directorio para resultados
mkdir -p results
RESULTS_FILE="results/scalability_results_$(date +%Y%m%d_%H%M%S).txt"

# Función para ejecutar una prueba
run_test() {
    local nprocs=$1
    local scale=$2
    echo "Ejecutando con $nprocs procesos y factor de escala $scale..."
    echo "-------------------------------------------------------------------" >> $RESULTS_FILE
    echo "Procesos: $nprocs, Factor de escala: $scale" >> $RESULTS_FILE
    echo "-------------------------------------------------------------------" >> $RESULTS_FILE
    mpirun -np $nprocs python knn_digits_parallel.py $scale >> $RESULTS_FILE 2>&1
    echo "" >> $RESULTS_FILE
}

# Prueba 1: Escalabilidad fuerte (datos fijos, aumentar procesos)
echo "PRUEBA 1: Escalabilidad Fuerte (datos fijos, variar procesos)"
echo "==================================================================="
echo "PRUEBA 1: ESCALABILIDAD FUERTE" >> $RESULTS_FILE
echo "==================================================================" >> $RESULTS_FILE

FIXED_SCALE=10  # Factor de escala fijo
for p in 1 2 4 8 16
do
    run_test $p $FIXED_SCALE
done

# Prueba 2: Escalabilidad débil (aumentar datos proporcionalmente a procesos)
echo ""
echo "PRUEBA 2: Escalabilidad Débil (datos proporcionales a procesos)"
echo "==================================================================="
echo "" >> $RESULTS_FILE
echo "PRUEBA 2: ESCALABILIDAD DÉBIL" >> $RESULTS_FILE
echo "==================================================================" >> $RESULTS_FILE

for p in 1 2 4 8 16
do
    scale=$((p * 2))  # Escalar datos proporcionalmente
    run_test $p $scale
done

# Prueba 3: Variación del tamaño de datos con procesos fijos
echo ""
echo "PRUEBA 3: Variación del tamaño de datos (procesos fijos)"
echo "==================================================================="
echo "" >> $RESULTS_FILE
echo "PRUEBA 3: VARIACIÓN DE DATOS" >> $RESULTS_FILE
echo "==================================================================" >> $RESULTS_FILE

FIXED_PROCS=4  # Número fijo de procesos
for scale in 1 5 10 20 50
do
    run_test $FIXED_PROCS $scale
done
...
# Crear un script Python para analizar los resultados
...
echo "Script de análisis creado: analyze_results.py"
echo "Para analizar los resultados ejecute:"
echo "  python analyze_results.py $RESULTS_FILE"